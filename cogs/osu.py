...
class osu(commands.Cog, name='osu!'):
    ...
    async def compare(self, ctx, user = None):
        ...
        for index, play in enumerate(tops):
            ...
            status = self.nomstat[status]
            if play['pp'] is None:
                beatmapDict, playDict = ppc.calculatePlay(bmap, mode, int(play["countmiss"]), int(play["count50"]), int(play["count100"]), int(play["count300"]), int(play["countgeki"]), int(play["countkatu"]), int(play["maxcombo"]), int(play["enabled_mods"]), int(play["perfect"]))
                pp = playDict["pp"]
            else:
                pp = round(float(play['pp']), 2)
