...
class Image(commands.Cog):
    ...
    async def wink(self, ctx):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://some-random-api.ml/animu/wink') as r:
                res = await r.json()
                imgUrl = res['link']
        embed = discord.Embed(title=f'{ctx.author.name} winks 😉', color=self.bot.config.color).set_image(url=imgUrl)
        await ctx.send(embed=embed)
    ...
    async def pat(self, ctx, user: discord.Member = None):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://some-random-api.ml/animu/pat') as r:
                res = await r.json()
                imgUrl = res['link']
        embed = discord.Embed(title=f'{ctx.author.name} pats {f"themselves. 😔" if user is None else f"{user.name} <:angery:545969728527663114>"}', color=self.bot.config.color).set_image(url=imgUrl)
        await ctx.send(embed=embed)
    ...
    async def hug(self, ctx, user: discord.Member = None):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://some-random-api.ml/animu/hug') as r:
                res = await r.json()
                imgUrl = res['link']
        embed = discord.Embed(title=f'{ctx.author.name} hugs {f"themselves. 😔" if user is None else f"{user.name} <:angery:545969728527663114>"}', color=self.bot.config.color).set_image(url=imgUrl)
        await ctx.send(embed=embed)
    ...
    async def avatar(self, ctx, user : discord.Member = None):
        ...
        embed = discord.Embed(title=f"{ctx.author.name if user is None else user.name}'s avatar.", color=self.bot.config.color).set_image(url=ctx.author.avatar_url if user is None else user.avatar_url)
        await ctx.send(embed=embed)
