...
class osuAPI:
    ...
    async def getuser(
        ...
        async with aiohttp.ClientSession() as cs:
            params = {"k": self.__token, "u": usr, "m": mode.id, "type": qtype}
    ...
    async def getbmap(
        ...
        async with aiohttp.ClientSession() as cs:
            params = {"k": self.__token, "mods": int(mods), "limit": limit, "a": 1}
    ...
    async def getrecent(self, user: osu.User, limit: int = 1) -> osu.RecentScore:
        async with aiohttp.ClientSession() as cs:
            params = {
                "k": self.__token,
                "u": user.user_id,
