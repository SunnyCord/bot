...
class BeatmapEmbed(discord.Embed):
    def __init__(self, beatmap: osu.Beatmap, color: str):
        ...
        date = (
            beatmap.last_update if not beatmap.approved_date else beatmap.approved_date
        )
        ...
        self.set_author(
            name=f"Mapped by {beatmap.creator}",
            icon_url=f"https://a.ppy.sh/{beatmap.creator_id}",
            url=f"https://osu.ppy.sh/users/{beatmap.creator_id}/modding",
        )
        self.set_thumbnail(url=f"https://b.ppy.sh/thumb/{beatmap.beatmapset_id}.jpg")
        self.set_footer(
            text=f"{beatmap.status.name} | ❤️ {beatmap.favourite_count} | ▶️ {beatmap.playcount} | {footerDate}",
        )
        self.add_field(
            name=f"{getDiffEmoji(beatmap.difficultyrating, int(beatmap.mode)).value}__{beatmap.version}__",
            value=f"**▸** {beatmap.difficultyrating:.2f}⭐  **▸Max Combo:** x{beatmap.max_combo}\n**▸AR:** {beatmap.diff_approach}  **▸OD:** {beatmap.diff_overall}  **▸CS:** {beatmap.diff_size}  **▸HP:** {beatmap.diff_drain}",
        )
