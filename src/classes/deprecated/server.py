...
class Server(Enum):
    ...
    def from_id(cls, id: int) -> Server:
        for item in cls:
            if item.value[0] == id:
                return item
        raise ValueError(f"{id!r} is not a valid {cls.__name__} id")
