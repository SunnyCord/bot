...
class OsuRecentEmbed(discord.Embed):
    ...
    def __init__(self, score:osu.Score, beatmap:osu.Beatmap, color: str):
        ...
        description = f"""
> {score.rank.icon} > **{round(score.performance.pp, 2)}PP""" + (f"({round(score.performance.pp_fc, 2)}PP for {round(score.performance.accuracy_fc * 100, 2)}% FC)" if not score.perfect else "") + f"""** > {round(score.performance.accuracy, 2)}%
